...
# === ConvNeXt-Tiny Model ===
def load_convnext_model(class_count):
    model = models.convnext_tiny(weights='IMAGENET1K_V1')
    for param in model.features.parameters():
        param.requires_grad = False
    model.classifier[2] = nn.Linear(model.classifier[2].in_features, class_count)
...
for day in ['Day 3', 'Day 4', 'Day 5']:
    ...
    # Using ConvNeXt-Tiny Model
    model = load_convnext_model(len(class_names))
    ...
    print(f"\n👉 Finished training for {day}. If satisfied, save manually using:\n"
          f"torch.save(model.state_dict(), '{day.replace(' ', '_')}_convnext_model.pth')")
    ...
    print(f"✅ Results for {day} (ConvNeXt-Tiny) saved to {summary_file}")
