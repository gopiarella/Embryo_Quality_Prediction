...
def load_efficientnet_model(class_count):
...
for day in ['Day 3', 'Day 4', 'Day 5']:
    ...
    model = load_efficientnet_model(len(class_names))
    ...
    print(f"\n👉 Finished training for {day}. If satisfied, save manually using:\n"
          f"torch.save(model.state_dict(), '{day.replace(' ', '_')}_efficientnet_model.pth')")
    ...
    print(f"✅ Results for {day} (EfficientNet-B0) saved to {summary_file}")
