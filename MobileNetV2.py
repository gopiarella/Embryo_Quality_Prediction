...
# === ResNet18 Model ===
def load_resnet_model(class_count):
...
# === MobileNetV2 Model ===
def load_mobilenet_model(class_count):
...
for day in ['Day 3', 'Day 4', 'Day 5']:
    ...
    if model_type == "resnet":
        model = load_resnet_model(len(class_names))
    elif model_type == "mobilenet":
        model = load_mobilenet_model(len(class_names))
    else:
    ...
    print(f"\n👉 Finished training for {day}. If satisfied, save manually using:\n"
          f"torch.save(model.state_dict(), '{day.replace(' ', '_')}_{model_type}_model.pth')")
    ...
    print(f"✅ Results for {day} ({model_type}) saved to {summary_file}")
